...
export type Photo = { id:string; title:string; description:string; imageUrl:string; slideshowImageUrl:string; instagramUrl:string; locationName:string; countryName:string; isFilm:boolean; takenOn:string|null; lat:number; lng:number; createdAt:string };
export type PhotoDraft = { title:string; description:string; imageUrl:string; slideshowImageUrl:string; instagramUrl:string; locationName:string; countryName:string; isFilm:boolean; takenOn:string|null; lat:number; lng:number };
...
function mapPhoto(row: { id:string; title:string; description:string; imageUrl:string; slideshowImageUrl:string; instagramUrl:string; locationName:string; countryName:string; isFilm:boolean; takenOn:Date|null; lat:number; lng:number; createdAt:Date }): Photo {
  return { id:row.id, title:row.title, description:row.description, imageUrl:normalizeImageUrl(row.imageUrl), slideshowImageUrl:normalizeImageUrl(row.slideshowImageUrl), instagramUrl:row.instagramUrl, locationName:row.locationName, countryName:row.countryName, isFilm:row.isFilm, takenOn:row.takenOn?.toISOString().slice(0,10) ?? null, lat:row.lat, lng:row.lng, createdAt:row.createdAt.toISOString() };
}
function draftData(draft: PhotoDraft) { return { title:draft.title.trim(), description:draft.description.trim(), imageUrl:normalizeImageUrl(draft.imageUrl), slideshowImageUrl:normalizeImageUrl(draft.slideshowImageUrl), instagramUrl:draft.instagramUrl.trim(), locationName:draft.locationName.trim(), countryName:draft.countryName.trim(), isFilm:draft.isFilm, takenOn:draft.takenOn ? new Date(`${draft.takenOn}T00:00:00.000Z`) : null, lat:draft.lat, lng:draft.lng }; }
